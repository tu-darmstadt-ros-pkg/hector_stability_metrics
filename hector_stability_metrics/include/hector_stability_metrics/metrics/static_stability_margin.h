...
template<typename Scalar>
size_t computeStaticStabilityMarginLeastStableEdgeIndex(
...
typename MinimumSelector::ReturnType computeStaticStabilityMargin(
    const math::Vector3List<Scalar> &support_polygon, std::vector<Scalar> &edge_stabilities,
    const math::Vector3<Scalar> &center_of_mass, Scalar normalization_factor = Scalar( 1 ) )
{
  ...
  for ( int i = 0; i < number_of_edges; i++ ) {
    ...
    const Scalar signed_distance_to_edge =
        ( projected_edge.normalized().cross( projected_edge_point - projected_com ) )( 2 );
